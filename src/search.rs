...
fn search<NODE: NodeType>(
    ...
    if !NODE::ROOT && !excluded && potential_singularity && ply < 2 * td.root_depth as isize {
        ...
        if score < singular_beta {
            let double_margin =
                -4 + 256 * NODE::PV as i32 - 16 * tt_move.is_quiet() as i32 - 32 * correction_value.abs() / 128;
            let triple_margin =
                48 + 288 * NODE::PV as i32 - 16 * tt_move.is_quiet() as i32 - 32 * correction_value.abs() / 128;
